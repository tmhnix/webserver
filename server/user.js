...
exports.security = function(req, res) {
    ...
    if (!user.mfa_secret) {
        ...
        var qrUri = 'otpauth://totp/tmhnix:' + user.username + '?secret=' + user.mfa_potential_secret + '&issuer=tmhnix';
...
exports.enableMfa = function(req, res, next) {
    ...
    if (otp !== expected) {
        ...
        var qrUri = 'otpauth://totp/tmhnix:' + user.username + '?secret=' + secret + '&issuer=tmhnix';
        user.qr_svg = qr.imageSync(qrUri, {type: 'svg'});
        user.sig = sig;
